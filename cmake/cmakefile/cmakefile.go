...
// Amalgamate amalgamates files in sources into dest.
func (cmake *CMakeFile) Amalgamate(dest string, sources []string) {
	basename := fmt.Sprintf("MK_AMALGAMATE_%d", cmake.output.Len())
	cmake.WriteLine(fmt.Sprintf(
		`file(WRITE "%s" "// AUTOGENERATED BY CMake! DON'T EDIT!\n\n")`, dest))
	for idx, source := range sources {
		name := fmt.Sprintf("%s_%d", basename, idx)
		cmake.WriteLine(fmt.Sprintf(`file(READ "%s" %s)`, source, name))
		cmake.WriteLine(fmt.Sprintf(
			`file(APPEND "%s" "${%s}")`, dest, name))
		if idx < len(sources)-1 {
			cmake.WriteLine(fmt.Sprintf(`file(APPEND "%s" "\n\n")`, dest))
		}
	}
}
